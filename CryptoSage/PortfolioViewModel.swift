...
class PortfolioViewModel: ObservableObject {
    ...
    init(service: PortfolioDataService = MockPortfolioDataService(initialHoldings: PortfolioViewModel.sample.holdings),
        ...
        // Reactively fetch live prices for current holdings and update prices
        $holdings
            .map { $0.map(\.coinSymbol).sorted() }      // list of symbols
            .removeDuplicates()
            .flatMap { symbols in
                priceService.pricePublisher(for: symbols, interval: 60)
            }
            .combineLatest($holdings)
            .map { prices, holdings in
                holdings.map { h in
                    var updated = h
                    if let livePrice = prices[h.coinSymbol] {
            ...
            .receive(on: DispatchQueue.main)
            .sink { [weak self] updatedHoldings in
