...
struct SubscriptionPricingView: View {
    // Customizable theme colors for reuse
    var accentColor: Color = .theme.accent
    var backgroundColor: Color = .theme.background
    var cardBackgroundColor: Color = .theme.cardBackground.opacity(0.85)
    var secondaryColor: Color = .theme.secondary
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 24) {
                    ForEach(tiers) { tier in
                        PricingCard(tier: tier, annual: annualBilling, accentColor: accentColor, secondaryColor: secondaryColor, cardBackgroundColor: cardBackgroundColor)
                            .padding(.horizontal)
                    }
                }
                .padding(.bottom)
