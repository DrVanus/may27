...
struct HomeView: View {
    ...
    private var portfolioSummaryHeader: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Portfolio")
                .font(.title3).bold()
                .foregroundColor(.white)
            Text(portfolioVM.totalValue, format: .currency(code: "USD"))
    ...
    private var homeContentStack: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            // Distribution / History card with drill-in
            ZStack(alignment: .topTrailing) {
                PortfolioChartView(portfolioVM: portfolioVM, showMetrics: false)
                    .padding(.vertical, 12)
                    .padding(.horizontal, 16)
                    .background(
                        RoundedRectangle(cornerRadius: 10)
