...
struct TradeView: View {
    ...
    private func bidsColumn(geometry: GeometryProxy) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            ...
            ForEach(orderBookVM.bids.prefix(10), id: \.price) { bid in
                ZStack(alignment: .leading) {
                    Rectangle()
                        .fill(Color.green.opacity(highlightedBids.contains(bid.price) ? 0.5 : 0.3))
                        .frame(width: geometry.size.width * bidDepth(bid), height: 20)
                        .cornerRadius(4)
                        .animation(.easeInOut(duration: 0.5), value: highlightedBids)
    ...
    private func asksColumn(geometry: GeometryProxy) -> some View {
        VStack(alignment: .leading, spacing: 2) {
            Text("Asks")
                .font(.caption2).bold()
                .foregroundColor(.red)
            ForEach(orderBookVM.asks.prefix(10), id: \.price) { ask in
                ZStack(alignment: .trailing) {
                    Rectangle()
                        .fill(Color.red.opacity(highlightedAsks.contains(ask.price) ? 0.5 : 0.3))
                        .frame(width: geometry.size.width * askDepth(ask), height: 20)
                        .cornerRadius(4)
                        .animation(.easeInOut(duration: 0.5), value: highlightedAsks)
...
private var navBar: some View {
    ...
    private var depthList: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            GeometryReader { geometry in
                HStack(alignment: .top, spacing: 16) {
                    bidsColumn(geometry: geometry)
                        .frame(width: geometry.size.width * 0.45, alignment: .leading)
                    asksColumn(geometry: geometry)
                        .frame(width: geometry.size.width * 0.45, alignment: .leading)
                    Spacer()
                }
                .padding(.horizontal, 12)
    ...
    // MARK: - Best Bid/Ask Summary Chips
    private var bestSummaryChips: some View {
        HStack(spacing: 12) {
            let bestBid = orderBookVM.bids.first?.price ?? "--"
            let bestAsk = orderBookVM.asks.first?.price ?? "--"
            HStack(spacing: 4) {
                Text("Bid").font(.caption2).bold()
                Text(bestBid).font(.caption2)
            }
            .foregroundColor(.green)
            ...
            HStack(spacing: 4) {
                Text("Ask").font(.caption2).bold()
                Text(bestAsk).font(.caption2)
            }
            .foregroundColor(.red)
            .padding(.vertical, 4).padding(.horizontal, 8)
