...
class PortfolioViewModel: ObservableObject {
    ...
    // Service providing holdings & transactions
    private let dataService: PortfolioDataService

    // Service providing live price updates
    private let priceService: PriceService

    // Combine cancellables
    private var cancellables = Set<AnyCancellable>()

    /// Initialize with a data service (mock or live) and a price service.
    init(service: PortfolioDataService = MockPortfolioDataService(initialHoldings: PortfolioViewModel.sample.holdings),
         priceService: PriceService = CoinGeckoPriceService()) {
        self.dataService = service
        self.priceService = priceService

        // Subscribe to holdings updates
        dataService.holdingsPublisher
            .receive(on: DispatchQueue.main)
            .assign(to: \.holdings, on: self)
            .store(in: &cancellables)

        // Subscribe to transactions updates
        dataService.transactionsPublisher
            .receive(on: DispatchQueue.main)
            .assign(to: \.transactions, on: self)
            .store(in: &cancellables)

        // Subscribe to price updates every 15s and rebuild history
        priceService
            .pricePublisher(for: holdings.map { $0.coinSymbol }, interval: 15)
            .receive(on: DispatchQueue.main)
            .sink { [weak self] priceMap in
                guard let self = self else { return }
                self.holdings = self.holdings.map { h in
                    var copy = h
                    if let newPrice = priceMap[h.coinSymbol.lowercased()] {
                        copy.currentPrice = newPrice
                    }
                    return copy
                }
                self.loadHistory()  // rebuild chart history based on updated prices
            }
            .store(in: &cancellables)
    }
