    ...
    private var metricsVStack: some View {
        VStack(alignment: .leading, spacing: 4) {
            ...
            Text("Total Value")
                ...
                .font(.subheadline)
            HStack(spacing: 12) {
                Text(viewModel.dailyChangePercentString)
                    .font(.caption)
                    .foregroundColor(viewModel.dailyChangePercent >= 0 ? .green : .red)
                Text(viewModel.unrealizedPLString)
                    .font(.caption)
                    .foregroundColor(viewModel.unrealizedPL >= 0 ? .green : .red)
            }
        }
        .padding(.leading, 16)
    }
            ...
            Group {
                if showLegend {
                    VStack(alignment: .leading, spacing: 8) {
                        ForEach(viewModel.allocationData, id: \.symbol) { slice in
                            HStack(spacing: 6) {
                                Circle()
                                    .fill(viewModel.color(for: slice.symbol))
    ...
    private var performanceSection: some View {
        HStack(spacing: 24) {
            VStack(alignment: .leading, spacing: 2) {
                ...
                Text(viewModel.topPerformerString)
                    .font(.headline)
                    .foregroundColor(viewModel.topPerformerString.contains("+") ? .green : .red)
            }
            VStack(alignment: .leading, spacing: 2) {
                Text("Worst Performer")
                    .font(.caption2)
                    .foregroundColor(.gray)
                Text(viewModel.worstPerformerString)
                    .font(.headline)
                    .foregroundColor(viewModel.worstPerformerString.contains("-") ? .red : .green)
            }
        }
        .padding(.trailing, 16)
    }
